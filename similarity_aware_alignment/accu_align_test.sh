...
# accu_align_test.sh
#
# This is a test script, not a production script.
#
...
# 2018.09.14 Brian Yu Testing what happens when bowtie2 only returns single best alignemnts
#                     Uses the script tabulate_alignment_fragment_test.py
#                     Do not generate multiple aligned reads files. Store only alignment summary.
#####################################################################
...
# 2018.09.14 CHANGED TO OUTPUT THE BEST ALIGNMENT ONLY. FOR TESTING PURPOSES
# bowtie2 -X $maxInsert -k $maxAlignment --threads $coreNum -t -x $bowtie2GenomeBase -1 $tempFolder/read1_trimmed.fastq.gz -2 $tempFolder/read2_trimmed.fastq.gz | samtools view -bh -f 0x003 -o $tempFolder/proper_alignment.bam -
bowtie2 -X $maxInsert --threads $coreNum -t -x $bowtie2GenomeBase -1 $tempFolder/read1_trimmed.fastq.gz -2 $tempFolder/read2_trimmed.fastq.gz | samtools view -bh -f 0x003 -o $tempFolder/proper_alignment.bam -
...
date
# python $scriptFolder/tabulate_alignment_fragment_v2.py $referenceNameFile $tempFolder/unique_alignment_sortedByName.bam $tempFolder/multiple_alignment_sortedByName.bam $tempFolder/tabulated_alignment_fragment.csv $sampleName
python $scriptFolder/tabulate_alignment_fragment_test.py $referenceNameFile $tempFolder/proper_alignment_sortedByName.bam $tempFolder/tabulated_alignment_fragment.csv $sampleName
date
