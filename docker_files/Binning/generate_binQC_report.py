...
def classify_method_stats(row):
    ...
    if row["classification_method"] == "ANI/Placement":
        ...
        row["closest_ref"] = row["closest_placemqent_reference"]
    else:
        row["ani"] = None
        row["aln_frac"] = None
...
def parse_checkm_output(checkm_summary):
    df = pd.read_table(checkm_summary).rename(
        columns={"Bin Id": "bin_name", "Strain heterogeneity": "Strain_heterogeneity"}
    )
    ...
    return (
        df[keep_cols].sort_values("checkm_score", ascending=True).reset_index(drop=True)
    )
...
def parse_gene_stats(genes_file):
    return pd.read_table(
        genes_file, names=["bin_name", "num_genes"], index_col="bin_name"
    )
...
if __name__ == "__main__":
    ...
    db_stats_df = (
        pd.read_csv(database_basic_stat)
        .rename(columns={"GenomeName": "bin_name"})
        .set_index("bin_name")
    )
    ...
    bins_df = (
        ...
        .sort_values("checkm_score", ascending=True)
        .reset_index()
        .rename(columns={"index": "bin_name"})
    )
